...
from registration.signals import post_user_registration_validation,\
    ...
    def validate(self, data):
        ...
        if reg_profile != user.registration_profile:
            raise ValidationError(
                message='The code does not belong to this email!')
...
class PasswordResetSerializer(serializers.Serializer):
    email = serializers.EmailField(
        label='Password Reset E-Mail Address', validators=[email_does_exist])
    ...
    def send_password_reset_email(self):
        ...
        email_to_send = EmailMessage(
            subject='Discovery password reset',
            body=f'Hi \n\nThis is your code for resetting your password on Discovery: {user.registration_profile.code}\n\n'
                 f'Please proceed with the sign-up process.\n\n'
                 f'Your Discovery team',
            to=[f"{email}"],
        )
...
class PasswordResetValidationSerializer(serializers.Serializer):
    code = serializers.CharField(
        label='Validation code', write_only=True, validators=[code_is_valid])
    email = serializers.EmailField(
        label='Registration E-Mail Address', validators=[email_does_exist])
    password = serializers.CharField(label='password', write_only=True)
    password_repeat = serializers.CharField(
        label='password_repeat', write_only=True)
