...
class User(AbstractUser):
    ...
    medical_field = models.CharField(
        verbose_name="medical field", max_length=200, blank=True
    )
    ...
    def friends(self):
        ...
        requested_requests = Friend.objects.filter(requester=self, status="A")
    ...
    def friend_requests_received(self):
        ...
        received_requests = Friend.objects.filter(receiver=self, status="P")
        for friend in received_requests:
            friends_profiles.append(friend.requester)
        return friends_profiles
    ...
    def friend_requests_sent(self):
        ...
        requested_requests = Friend.objects.filter(requester=self, status="P")
        for friend in requested_requests:
            friends_profiles.append(friend.receiver)
        return friends_profiles
    ...
    def friend_requests_sent_rejected(self):
        ...
        rejected_requests = Friend.objects.filter(requester=self, status="R")
